...
function getStorageInfo(callback) {
    if (os.platform() === "win32") {
        exec("wmic logicaldisk get size,freespace,caption", (err, stdout) => {
            ...
            const infoPenyimpanan = lines
                .map(line => {
                    const [drive, free, total] = line.trim().split(/\s+/);
                    return `🖥️ ${drive}: ${(total / 1024 ** 3).toFixed(
                        2
                    )} GB total, ${(free / 1024 ** 3).toFixed(2)} GB bebas`;
                })
                .join("\n");
        ...
        exec("df -h --output=source,size,avail,target", (err, stdout) => {
            ...
            const infoPenyimpanan = lines
                .map(line => {
                    const [device, total, free, mount] = line
                        .trim()
                        .split(/\s+/);
