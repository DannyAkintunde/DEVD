...
const { getRandom, getFullFilePath, fetchJson, getBuffer, json } = require("../lib/functions");
...
cmd(
    ...
    async (
        ...
        try {
            const image = m.getImage();
            const video = m.getVideo();
            const sticker = m.getSticker();
            if (image) {
                ...
                if (getVideoDuration(video) > maxDuration) {
                  return reply(`Use a video that has a duration less than or equal to ${maxDuration} seconds.`);
                }
                ...
                return conn.sendMessage(
                    from,
                    await stickerObj.toMessage(),
                    { quoted: mek }
                );
            } else return await reply("*Please reply to a photo, video or existing sticker!*");
        } catch (e) {
            m.sendError(e);
        }
    }
