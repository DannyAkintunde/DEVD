...
const { getRandom, getFullFilePath } = require("./functions");
const { convertTemplateToES6 } = require("./templar.js");
...
const sms = (conn, m) => {
    ...
    if (m.message) {
        ...
        if (m.msg) {
            ...
            var mention =
                typeof tagMention == "string" ? [tagMention] : tagMention;
            ...
            if (m.quoted) {
                ...
                if (m.quoted.type === "viewOnceMessage") {
                    m.quoted.msg.type = getContentType(
                        m.quoted[m.quoted.type].message
                    );
                }
                ...
                m.quoted.body =
                    m.quoted.type === "conversation"
                        ...
                        ? m.quoted.msg.text
                        : m.quoted.type == "imageMessage" && m.msg.caption
                        ? m.quoted.msg.caption
                        : m.quoted.type == "videoMessage" && m.msg.caption
                        ? m.quoted.msg.caption
        ...
        m.getImage = (quoted = true) => getImageFromMsg(m, quoted);
        m.getVideo = (quoted = true) => getVideoFromMsg(m, quoted);
        m.getSticker = () => getStickerFromMsg(m);
    ...
    m.replyAud = (
        aud,
        id = m.chat,
        option = { mentions: [m.sender], ptt: false, mimetype: "audio/mpeg" }
    ) =>
        conn.sendMessage(
            id,
            {
    ...
    m.sendError = (e, msg = "", key = null) => {
        if (!config.LOG) {
            conn.sendMessage(
                m.chat,
                {
                    text: convertTemplateToES6(msg || global.responses.error),
                    ...(key ? { edit: key } : {})
                },
                { quoted: m }
            );
            ...
            conn.sendMessage(
                m.chat,
                {
                    ...
                    contextInfo: {
                        ...
                        externalAdReply: {
                            title: `「 ERROR 」`,
                            body: convertTemplateToES6(
                                msg || global.responses.error
                            ),
                            mediaType: 1,
                            sourceUrl: global.link,
                            thumbnailUrl:
                                "https://telegra.ph/file/fa42ea5b3545fc06a868c.jpg",
...
const getImageFromMsg = async (m, quoted = true) => {
    ...
    const isQuotedImage = m.quoted
        ...
        : false;
    if (m.type === "imageMessage" || (isQuotedImage && quoted)) {
...
const getVideoFromMsg = async (m, quoted = true) => {
    const isQuotedViewOnce = m.quoted
        ? m.quoted.type === "viewOnceMessage"
        : false;
    const isQuotedVideo = m.quoted
        ...
        : false;
    if (m.type === "videoMessage" || (isQuotedVideo && quoted)) {
        var nameMp4 = getRandom("");
        video = isQuotedVideo
            ? await m.quoted.download(nameMp4)
            : await m.download(nameMp4);
        return video;
    }
};

const getStickerFromMsg = async m => {
    const isQuotedViewOnce = m.quoted
        ? m.quoted.type === "viewOnceMessage"
        : false;
    const isQuotedSticker = m.quoted
        ? m.quoted.type === "stickerMessage" ||
          (isQuotedViewOnce ? m.quoted.msg.type === "stickerMessage" : false)
        : false;
    if (isQuotedSticker) {
        var nameWebp = getRandom("");
        sticker = isQuotedSticker
            ? await m.quoted.download(nameWebp)
            : await m.download(nameWebp);
        return sticker;
    }
};
...
module.exports = { sms, downloadMediaMessage, getImageFromMsg, messageType };
