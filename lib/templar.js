...
const FUNCTIONS = {
    ...
    quotestr: (value, symbol = '"') => {
        if (isNaN(Number(value)) && !value.includes(symbol))
...
function parseTemplate(template) {
    ...
    if (stack.length > 0) {
        throw new SyntaxError("Mismatched #if/#endif tags in template.");
    }
...
function loadPlaceholders(template, data, quoted = false, skip_null = true) {
...
function convertTemplateToES6(template, data = DEFUALT_OBJ) {
    data = { ...DEFUALT_OBJ, ...data };
    //template = processConditionals(template, data);
    let result;
    result = loadPlaceholders(template, data);
...
// test syntax
/*
console.log(convertTemplateToES6(`
{% #for user in users %}
{% #if user %}
{{styles.BOLD}}Username{{styles.BOLD}}: {{user.name:capitalize}}
{{styles.BOLD}}Phone number{{styles.BOLD}}: {{styles.MONO}}{{user.phone}}{{styles.MONO}}
{% #endif %}
{% #endfor %}
`, {users: [{name: 'danny', phone: '2348098309204'}, {name: 'joshua', phone: '2348098309205'}]}))
*/