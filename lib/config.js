...
global.getApi = (ID, path = '/', query = {}) => (ID in APIs ? global.APIS[ID] : ID) + path + (query ? '?' + new URLSearchParams(Object.entries({...query})) : '')

// Init for fastapi
const fastKey = (async function getFastKey() {
  let key;
  fetch(global.getApi("fastapi","/api/register", {username: `${config.OWNER_NAME}-${config.OWNER_NUMBER}`})).then(res => {
    let data = res.text();
    key = data.split("\n")[1].slice(9).trim();
  })
  return key
