...
async function tikstalk(user) {
  let res = await axios.get(`https://urlebird.com/user/${user}/`, {headers: { "user-agent": (new UserAgent()).toString()}})
...
async function githubstalk(user) {
    return new Promise(async (resolve, reject) => {
      ...
      const mostStarredRepo = reposData.slice().sort((a, b) => b.stargazers_count - a.stargazers_count).slice(0, 5);
      ...
      let info = {
          ...
          public_repo_no: profileData.public_repos,
...
async function githubRoasting(username) {
  try {
    let roast = ''
    await githubstalk(username).then(async (githubProfileInfo) => {
...
/*
githubRoasting('siputzx').then(summary => console.log(summary)).catch(err => console.error(err));
*/
