...
const payload = `const { chromium } = require('playwright');
...
async function iask(query) {
    const code = payload + `iask('${query}').then(a => console.log(a));`;
    const start = await playwright.run(code);
    const string = start.output;
