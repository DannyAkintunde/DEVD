...
const mediaFireRegrex =
    /^((http|https):\/\/)?([A-z]{2,}\.)?(mediafire.com\/file)(.+)\/(.+)/g;
...
const mediaFire = {
    request: async url => {
        try {
            ...
            if (
                ...
                const downloadLink =
                    mediaFire.extractDownloadUrl(downloadButtonUrl);
                const sizeMatch = $("#downloadButton")
                    .text()
                    ?.match(/\(.+\)/);
                const size = sizeMatch
                    ? sizeMatch[0].slice(1, -1)
                    : "Unknown size";
                const name =
                    $('meta[property="og:title"]').attr("content") ||
                    "Unknown file";
                const fileName =
                    $(".dl-btn-label").attr("title") || "Unknown file name";
                const thumbnail = $('meta[property="og:image"]').attr(
                    "content"
                );
                const mimeType =
                    mime.lookup(fileName) || "aplication/octet-stream";
    ...
    extractDownloadUrl: url => {
        const downloadUrlRegrex =
            /((http|https):\/\/download[^\s&]+\.mediafire\.com[^\s&]+\/[^\s&]+)/;
