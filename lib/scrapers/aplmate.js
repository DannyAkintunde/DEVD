...
const aplMate = {
    getToken: async function() {
        ...
        const response = await axios.get('https://aplmate.com/', {'user-agent': this.userAgent.toString()});
    ...
    request: async function(url) {
        ...
        if (!rx.test(url)) {
            return { creator: 'Danny', status: 'error', code: 400, message: 'Link must be an album or song slug.' };
        }
        ...
        try {
            const response = await axios.post('https://aplmate.com/action', form, { headers: { ...form.getHeaders(), cookie, 'user-agent': this.userAgent ? this.userAgent.toString() : 'Postify/1.0.0' } });
            const $ = cheerio.load(response.data);
            ...
            if (url.includes('/song/')) {
                ...
                const artist = $('.aplmate-downloader p').text().trim().replace(/ · | · |\s/g, '').trim();
                ...
                return { 
                    ...
                    status: 'success', 
                    code: 200, 
                    data: [{ image, title, artist, download: downloadLinks, ...musicInfo }] 
                };
            }

            const results = await Promise.all(
                $('.row.dlvideos').map(async (index, element) => {
                    const image = $(element).find('img').attr('src');
                    const title = $(element).find('h3[itemprop=name] .hover-underline').text().trim();
                    const artist = $(element).find('p').text().trim().replace(/ · | · |\s/g, '').trim();
                    const jwtToken = $(element).find('input[name=data]').val();
                    const base = $(element).find('input[name=base]').val();
                    const tokens = $(element).find('input[name=token]').val();

                    const result = await this.getTrackData(jwtToken, base, tokens, cookie);
                    const download = result.error ? [] : this.getDownloadLinks(result.data);
            ...
            return { creator: 'Danny', status: 'success', code: 200, data: res };
        } catch {
            return { creator: 'Danny', status: 'error', code: 500, message: 'Internal Server Error' };
        }
    },

    getInfo: async function(url) {
        try {
            const response = await axios.get(url, {headers: "user-agent": this.userAgent ? this.userAgent.toString() : 'Postify/1.0.0'});
            const $ = cheerio.load(response.data);
            return {
    ...
    getTrackData: async function(jwtToken, base, tokens, cookie) {
        const form = new FormData();
        ...
        try {
            const response = await axios.post('https://aplmate.com/action/track', form, {
                headers: { ...form.getHeaders(), origin: 'https://aplmate.com', referer: 'https://aplmate.com/', cookie, 'user-agent': this.userAgent ? this.userAgent.toString() : 'Postify/1.0.0' },
            });
    ...
    getDownloadLinks: function(html) {
